...
void block4() 
{
    ...
    e = next_event_block_four(event); /* next event index */
    if (e != -1) {
        update_next_event(4, event[e].t, 1, event[s].disabled); /* There is a next event for this block, update the global_info */
    }
    else {
        update_next_event(4, INFINITY, -1, event[s].disabled);
    } 
}
