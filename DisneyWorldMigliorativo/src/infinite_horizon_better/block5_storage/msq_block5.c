...
#ifndef F
#define SERVERS_FIVE 8  /* number of servers 1 */
#else
#define SERVERS_FIVE 5  /* number of servers time slot 2 */
#endif
...
void block5() 
{
    ...
    if (sampling == 1) {
        ...
        if (processedJobs == 0) /* stats not yet ready */ {
            ...
            fprintf(fp,"%6.6f\n", avgnode);
            fclose(fp);
            fp = fopen(FILENAME_AVGQUEUE_BLOCK5, "a");
            fprintf(fp,"%6.6f\n", avgqueue);
            fclose(fp);
        }
        else {
    ...
    if (endSimulation == 1) {
        update_next_event(5, INFINITY, -1, -1);
        print_statistics();
        init = 1;
    ...
    if (e != -1) {
        update_next_event(5, event[e].t, 1, -1); /* There is a next event for this block, update the global_info */
    }
    else {
        update_next_event(5, INFINITY, -1, -1);
    }
