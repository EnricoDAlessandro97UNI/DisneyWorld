...
{   
    ...
    if (endSimulation == 1) {
        update_next_event(1, INFINITY, -1, -1);
        print_statistics();
    ...
    if (e != -1) {
        update_next_event(1, event[e].t, 1, event[e].disabled); /* There is a next event for this block, update the global_info */
    }
    else {
        update_next_event(1, INFINITY, 0, event[e].disabled); /* There isn't a next event for this block, update the global_info */
    }
